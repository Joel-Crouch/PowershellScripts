...
param(
    ...
    [int]$UpdateHour = 17,
    [string]$ExclusionGroup = "CN=GRP_NoRestart,OU=Computers,DC=domain,DC=net"
)
...
if ((Get-CimInstance -ClassName Win32_OperatingSystem).ProductType -eq 1){
    ...
    Try{
        $RestartTask=Get-ScheduledTask "IT - DelayedRestart - $env:USERNAME" -ErrorAction stop| Get-ScheduledTaskInfo
    }Catch{
    ...
    If ($DelayedRestart){
        ...
        Register-ScheduledTask -Settings $settings -Action $action -Trigger $trigger -TaskName "IT - DelayedRestart - $env:USERNAME" -Description "User delayed required restart" -Force -Verbose:$false | Out-Null
    ...
    if ($currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)){
        if ($CTask = Get-ScheduledTask -TaskName "IT - Get-PendingRestart Computer" -ErrorAction SilentlyContinue){
            Write-Verbose "Scheduled task for computer found on device. Removing." -Verbose
            $CTask | Unregister-ScheduledTask -Confirm:$false
        }
        if ($UTask = Get-ScheduledTask -TaskName "IT - Get-PendingRestarts User" -ErrorAction SilentlyContinue){
